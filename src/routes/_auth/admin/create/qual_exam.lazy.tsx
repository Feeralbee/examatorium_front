import {
  ...
  UsersService,
  GroupsService,
  CreateQualificationExamRequest,
} from "@client";
...
const CreateQualExamPage = () => {
  const form = useForm<CreateQualificationExamRequest>();
  const navigate = useNavigate();
  const mutation = useMutation({
    mutationFn: (data: CreateQualificationExamRequest) =>
      QualExamsService.createQualExamsPost({ requestBody: data }),
    onSuccess: () => navigate({ to: "/admin/qual_exams" }),
  });
  const onError: SubmitErrorHandler<CreateQualificationExamRequest> = () =>
  ...
  return (
    <>
      <form onSubmit={onSubmit}>
        <h1>Создание квалификационного экзамена</h1>
        <Grid
          container
          sx={{
            gap: 3,
...
export const Route = createLazyFileRoute("/_auth/admin/create/qual_exam")({
  component: CreateQualExamPage,
});
